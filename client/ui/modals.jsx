...
function ColdWalletForm({ intent, onSubmit, busy }) {
  ...
  return (
    <div className="crypto-pay-form">
      <div className="crypto-pay-head">
        <span className="pay-chip"></span>
        <div><b>CRYPTO CHECKOUT</b><span>Cold-wallet transfer</span></div>
        <span className="pay-secure">🔒 Direct wallet</span>
      </div>
      ...
      <div className="crypto-amount">
        <span>EXACT AMOUNT</span><strong>{intent.priceDisplay}</strong><small>Send the exact displayed amount in {wallet.asset || 'the selected asset'}.</small>
      </div>
      <div className="crypto-warning">⚠️ Send only on the <b>{wallet.network || 'specified network'}</b>. Sending another asset or using another network may result in permanent loss.</div>
      <div className="field">
        <label>TRANSACTION HASH *</label>
        <input value={txHash} onChange={e => setTxHash(e.target.value.trim())} maxLength="180" placeholder="Paste your transaction hash" autoComplete="off" />
      </div>
      <button className="btn btn-gold big" style={{ width: '100%' }} disabled={busy || !txHash.trim()} onClick={() => onSubmit(txHash.trim())}>
        {busy ? 'SUBMITTING…' : 'SUBMIT PAYMENT FOR VERIFICATION'}
      </button>
      <p className="muted small center" style={{ marginTop: '.65rem' }}>Your purchase is activated only after the transfer is manually verified. Do not send private keys or seed phrases.</p>
    </div>
  );
}
...
export function VoteModal({ slug }) {
  ...
  const go = async () => {
    ...
    try {
      ...
      actions.setSession(r); api('/api/my-votes').then(actions.setMyVotes).catch(() => { }); actions.closeModal(); actions.confetti(); emotionalToast(leader, r); actions.flashLeader(slug);
      setTimeout(() => actions.openModal('share', { slug, wantBonus: st.session.remaining === 0, afterVote: true }), 1600);
    } catch (err) { actions.closeModal(); actions.toast(VOTE_ERR[err.error] || 'Vote failed. Try again.', 'error'); if (err.error === 'no_votes_left') actions.openModal('vote', { slug }); }
  };
  return (
    <div>
      ...
      {max > 1 ? <div className="vote-spinner"><button aria-label="Fewer votes" onClick={() => setN(v => Math.max(1, v - 1))}>−</button><span className="vote-count">{n}</span><button aria-label="More votes" onClick={() => setN(v => Math.min(max, v + 1))}>+</button></div> : <div style={{ height: '0.8rem' }}></div>}
      <button className="btn btn-vote big" style={{ width: '100%' }} disabled={busy} onClick={go}>CAST {n} VOTE{n > 1 ? 'S' : ''}</button>
      <p className="muted small center" style={{ marginTop: '.6rem' }}>1 free vote per day · earn more by sharing (+1) or <button className="x-link" style={{ background: 'none', border: 'none', cursor: 'pointer', font: 'inherit' }} onClick={() => actions.openModal('buyvotes')}>buy vote packs</button></p>
    </div>
  );
}
...
export function BuyVotesModal() {
  ...
  const go = async txHash => {
    setBusy(true);
    try {
      const r = await api('/api/purchase/confirm', { method: 'POST', body: { intentId: intent.intentId, details: { txHash } } });
      if (r.status === 'pending_verification') { setSubmitted(true); return; }
      const st0 = getState(); actions.setSession({ remaining: r.remaining, purchased: (st0.session.purchased || 0) + r.votesAdded }); actions.closeModal(); actions.toast(`⚡ <b>+${r.votesAdded} votes</b> added!`, 'epic', 5500);
    } catch (e) { actions.toast(e.error || 'Payment submission failed', 'error'); setBusy(false); }
  };
  useEffect(() => {
    setIntent(null); setSubmitted(false);
    api('/api/purchase/intent', { method: 'POST', body: { kind: 'votes', reference: pack } }).then(setIntent).catch(e => { actions.toast(e.error || 'Could not start crypto checkout', 'error'); actions.closeModal(); });
  }, [pack]);
  ...
  return (
    <div>
      ...
      <p className="muted small">Choose a pack, send crypto to the cold wallet, then submit the transaction hash.</p>
      <div className="pack-grid">
        <button className={'pack' + (pack === 'votes-10' ? ' sel' : '')} onClick={() => setPack('votes-10')}><b>10</b><span>VOTES</span><span className="price">$1.00</span></button>
      ...
      <ColdWalletForm intent={intent} onSubmit={go} busy={busy} />
    </div>
  );
}
...
export function ShareModal({ slug, wantBonus = false, afterVote = false }) {
  ...
  const doShare = async platform => {
    try {
      const r = await api('/api/share', { method: 'POST', body: { slug, platform } }); const url = location.origin + r.shareUrl;
      ...
      if (r.bonusAwarded) { actions.setSession(r); setTimeout(() => actions.toast('🎁 <b>+1 BONUS VOTE</b> earned for sharing!', 'epic'), 700); }
  ...
  return (
    <div>
      {afterVote || wantBonus ? <><h3>Want 1 MORE vote? 🎁</h3><p className="muted small">Share {leader.name} and get <b>+1 bonus vote</b> instantly (max 3/day).</p></> : <><h3>Share {leader.name}</h3><p className="muted small">Every share can earn you +1 bonus vote.</p></>}
      <div className="terms-box" style={{ textAlign: 'center' }}><div style={{ fontSize: '1.6rem' }}>{leader.flag}</div><b>{leader.name}</b> · #{leader.rank} · {num(leader.total_votes)} votes<br /><span className="muted">"Do you agree? Vote now."</span></div>
      <div className="share-grid">{platforms.map(([id, ico, label]) => <button key={id} className="share-btn" onClick={() => doShare(id)}><span className="ico">{ico}</span>{label}</button>)}</div>
      {navigator.share ? <button className="btn btn-ghost" style={{ width: '100%' }} onClick={() => doShare('native')}>📲 More share options…</button> : null}
    </div>
  );
}
...
export function CheckoutModal({ kind, reference }) {
  ...
  const go = async txHash => {
    let details;
    if (kind === 'ad') {
      const name = rootRef.current.querySelector('#adName').value.trim(); if (!name) return actions.toast('Advertiser name is required', 'error');
      details = { advertiser: name, x_handle: rootRef.current.querySelector('#adX').value, text: rootRef.current.querySelector('#adText').value, cta: rootRef.current.querySelector('#adCta').value, url: rootRef.current.querySelector('#adUrl').value };
      ...
      const sponsor = rootRef.current.querySelector('#anName').value.trim(); if (!sponsor) return actions.toast('Sponsor name is required', 'error');
    ...
    try {
      ...
      if (r.status === 'pending_verification') { setSubmitted(true); return; }
      actions.closeModal(); actions.toast(`🏆 <b>Purchase complete!</b> ${esc(r.shareText || '')}`, 'epic', 6000);
      if (r.shareText && navigator.clipboard) navigator.clipboard.writeText(r.shareText + ' ' + location.origin).catch(() => { }); setTimeout(() => location.reload(), 2200);
    } catch (e) { actions.toast(e.error || 'Payment submission failed', 'error'); setBusy(false); }
  };
  ...
  return (
    <div ref={rootRef}>
      ...
      <div className="terms-box"><b>Item:</b> {t.item}<br /><b>Price:</b> {t.price}<br /><b>Ownership:</b> {t.duration}<br /><b>You receive:</b> {t.receives}<br /><b>Refunds:</b> {t.refunds}</div>
      {kind === 'ad' ? <>
        <div className="field"><label>COMPANY / ADVERTISER NAME *</label><input id="adName" maxLength="60" placeholder="Acme Inc." /></div>
        <div className="field"><label>𝕏 HANDLE</label><input id="adX" maxLength="16" placeholder="@acme" /></div>
        ...
        <div className="field"><label>YOUR NAME OR COMPANY *</label><input id="anName" maxLength="60" placeholder="John Doe" /></div>
        <div className="field"><label>𝕏 HANDLE</label><input id="anX" maxLength="16" placeholder="@johndoe" /></div>
      </>}
      <ColdWalletForm intent={intent} onSubmit={go} busy={busy} />
    </div>
  );
