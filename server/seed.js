...
function seedDemoVotes() {
  ...
  for (const l of leaders) {
    ...
    const base = Math.round(w * (38 + Math.random() * 28));
    let total = 0; const weekly = {};
    for (let d = 29; d >= 0; d--) {
      const trend = 1 + (Math.random() - 0.45) * 0.8;
      const dayVotes = Math.max(3, Math.round((base / 30) * trend * (0.4 + Math.random())));
      total += dayVotes;
      const day = dayStr(d); insDaily.run(l.id, day, dayVotes, Math.round(dayVotes * 0.06));
      const wk = day.slice(0, 8) + 'W'; weekly[wk] = (weekly[wk] || 0) + dayVotes;
      if (d < 2) {
...
function recomputeRanks(initial = false) {
  ...
  const changes = [];
  rows.forEach((r, i) => { const newRank = i + 1; if (r.rank !== newRank) changes.push({ id: r.id, from: r.rank, to: newRank }); upd.run(initial ? newRank : (r.rank ?? newRank), newRank, r.id); });
  return changes;
}
...
function seedRankHistory() {
  ...
  const byLeader = {}; daily.forEach(r => { (byLeader[r.leader_id] ||= []).push(r); });
  ...
  const perDay = {}; daily.forEach(r => { (perDay[r.day] ||= []).push(r); });
  for (const day of days) {
    (perDay[day] || []).forEach(r => cum[r.leader_id] += r.votes);
    leaders.map(l => ({ id: l.id, v: cum[l.id] })).sort((a, b) => b.v - a.v).forEach((o, i) => ins.run(o.id, day, i + 1, o.v));
  }
}
...
function clearDemoVotes() {
  const tx = db.transaction(() => {
    ...
    db.prepare('DELETE FROM leader_daily_stats').run(); db.prepare('DELETE FROM leader_weekly_stats').run(); db.prepare('DELETE FROM leader_rank_history').run();
...
function resetDemoData() {
  const tx = db.transaction(() => {
    ['votes','vote_sessions','bonus_votes','referrals','shares','leader_rank_history','leader_daily_stats','leader_weekly_stats','advertisements','ad_purchases','anthem_purchases','anthem_history','payments','activity_events','fraud_events'].forEach(t => db.prepare(`DELETE FROM ${t}`).run());
    db.prepare('DELETE FROM anthem_slots').run(); db.prepare('UPDATE leaders SET total_votes=0, rank=NULL, prev_rank=NULL').run(); db.prepare('UPDATE countries SET total_votes=0').run();
  }); tx(); seedDemoVotes();
}
