...
CREATE TABLE IF NOT EXISTS leaders (
  ...
  sort_order INTEGER DEFAULT 0, total_votes INTEGER DEFAULT 0, rank INTEGER, prev_rank INTEGER,
  created_at TEXT DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS vote_sessions (
  id TEXT PRIMARY KEY, session_id TEXT NOT NULL, day TEXT NOT NULL, ip TEXT, ua_hash TEXT,
  free_used INTEGER DEFAULT 0, bonus_earned INTEGER DEFAULT 0, bonus_used INTEGER DEFAULT 0,
  suspended INTEGER DEFAULT 0, last_share_at INTEGER DEFAULT 0, created_at TEXT DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS votes (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  leader_id INTEGER NOT NULL REFERENCES leaders(id) ON DELETE CASCADE,
  session_id TEXT, user_id INTEGER, type TEXT DEFAULT 'free', source TEXT DEFAULT 'web', country TEXT, ip_hash TEXT,
  created_at TEXT DEFAULT (datetime('now'))
);
...
CREATE TABLE IF NOT EXISTS shares (
  id TEXT PRIMARY KEY, session_id TEXT, leader_id INTEGER, platform TEXT, clicks INTEGER DEFAULT 0,
  created_at TEXT DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS leader_rank_history (
  id INTEGER PRIMARY KEY AUTOINCREMENT, leader_id INTEGER, day TEXT, rank INTEGER, votes INTEGER, UNIQUE(leader_id, day)
);
...
CREATE TABLE IF NOT EXISTS leader_weekly_stats (
  id INTEGER PRIMARY KEY AUTOINCREMENT, leader_id INTEGER, week TEXT, votes INTEGER DEFAULT 0, UNIQUE(leader_id, week)
);
CREATE TABLE IF NOT EXISTS advertising_slots (
  id TEXT PRIMARY KEY, label TEXT, price_usd REAL DEFAULT 5.0, active INTEGER DEFAULT 1
);
CREATE TABLE IF NOT EXISTS advertisements (
  id INTEGER PRIMARY KEY AUTOINCREMENT, slot_id TEXT REFERENCES advertising_slots(id), advertiser TEXT, image TEXT,
  text TEXT, cta TEXT, url TEXT, starts_at TEXT, ends_at TEXT, status TEXT DEFAULT 'active', created_at TEXT DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS ad_purchases (
  id INTEGER PRIMARY KEY AUTOINCREMENT, slot_id TEXT, ad_id INTEGER, payment_id INTEGER, advertiser TEXT,
  amount_usd REAL, created_at TEXT DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS anthem_slots (
  country_code TEXT PRIMARY KEY REFERENCES countries(code), sponsor TEXT, sponsor_session TEXT,
  price_usd REAL DEFAULT 5.0, purchased_at TEXT
);
...
CREATE TABLE IF NOT EXISTS fraud_events (
  id INTEGER PRIMARY KEY AUTOINCREMENT, kind TEXT, session_id TEXT, ip_hash TEXT, detail TEXT, created_at TEXT DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS site_settings (key TEXT PRIMARY KEY, value TEXT);
CREATE TABLE IF NOT EXISTS vote_idempotency (
  session_id TEXT NOT NULL, idempotency_key TEXT NOT NULL, request_hash TEXT NOT NULL, response_json TEXT NOT NULL,
