...
router.get('/session', (req, res) => {
  ...
  res.json({ remaining: core.remainingVotes(vs), free_used: vs.free_used, bonus_earned: vs.bonus_earned, bonus_used: vs.bonus_used,
...
router.post('/auth/login', rateLimit({ windowMs: 10 * 60_000, max: 10, name: 'auth-login' }), (req, res) => {
  ...
  if (!demoMode && provider !== 'local') return res.status(503).json({ error: 'oauth_not_configured', message: 'Social sign-in needs a real OAuth provider before launch.' });
  let user = null;
  ...
  if (user) {
    if (!demoMode) return res.status(409).json({ error: 'email_taken', message: 'This email is already registered. Sign-in verification is not available yet.' });
    ...
    const info = db.prepare('INSERT INTO users (email,display_name,provider,x_handle,avatar_color) VALUES (?,?,?,?,?)').run(email, name, provider, xh, color);
    user = db.prepare('SELECT * FROM users WHERE id=?').get(info.lastInsertRowid);
  }
  db.prepare('UPDATE vote_sessions SET user_id=? WHERE session_id=?').run(user.id, req.sessionId);
  db.prepare(`INSERT INTO vote_sessions (id,session_id,day,user_id) VALUES (?,?,date('now'),?)
              ON CONFLICT(id) DO UPDATE SET user_id=excluded.user_id`).run(req.sessionId + ':' + new Date().toISOString().slice(0, 10), req.sessionId, user.id);
...
router.post('/suggest-leader', rateLimit({ windowMs: 60 * 60_000, max: 3, name: 'suggest' }), async (req, res) => {
  const b = req.body || {};
  const name = cleanText(b.name, 60); const cc = String(b.country_code || '').trim().toUpperCase();
  if (name.length < 2) return res.status(400).json({ error: 'name_required' });
  ...
  if (db.prepare(`SELECT 1 FROM leaders WHERE suggested_by=? AND date(created_at)=?`).get(req.sessionId, today)) return res.status(429).json({ error: 'daily_limit', message: 'You can add 1 leader per day.' });
  ...
  db.prepare(`INSERT INTO leaders (slug,name,country_code,status,categories,era,years,title,bio,visible,verified,community,suggested_by) VALUES (?,?,?,?,?,?,?,?,?,0,0,1,?)`)
    .run(slug, name, cc, b.status === 'current' ? 'current' : 'historical', JSON.stringify(['political']), '', cleanText(b.years, 30), cleanText(b.title, 80), cleanLong(b.bio, 300), req.sessionId);
  recomputeRanks();
  core.pushActivity('leader', `🆕 ${core.FLAG(cc)} ${name} was submitted by the community for review`, cc, null);
  res.status(202).json({ ok: true, slug, status: 'pending_review' });
});

router.get('/stream', sse.handler);
router.get('/leaderboard', (req, res) => {
  const limitRaw = Number.parseInt(req.query.limit, 10); const offsetRaw = Number.parseInt(req.query.offset, 10);
  const limit = Math.max(1, Math.min(50, Number.isFinite(limitRaw) ? limitRaw : 10));
  const offset = Math.max(0, Math.min(100000, Number.isFinite(offsetRaw) ? offsetRaw : 0));
  ...
  res.json(core.leaderboard({ limit, offset, category, country }));
});
...
router.get('/country/:code', (req, res) => { const c = core.countryInfo(req.params.code); c ? res.json(c) : res.status(404).json({ error: 'not_found' }); });
...
router.get('/currency', (req, res) => { const ccy = req.query.ccy || currency.guessCurrency(req.headers['accept-language']); res.json({ base: 'USD', display: currency.display(5, ccy), currency: ccy, rates: currency.rates }); });

router.post('/vote', rateLimit({ windowMs: 60_000, max: 30, name: 'vote-endpoint' }), (req, res) => {
  const key = String(req.get('Idempotency-Key') || '');
  if (!IDEMPOTENCY_RE.test(key)) return res.status(400).json({ error: 'idempotency_key_required' });
  const body = req.body || {};
  const slug = String(body.slug || '');
  const count = Math.max(1, Math.min(10, Number.parseInt(body.count, 10) || 1));
  const requestHash = crypto.createHash('sha256').update(JSON.stringify({ slug, count })).digest('hex');
  const existing = db.prepare('SELECT request_hash,response_json FROM vote_idempotency WHERE session_id=? AND idempotency_key=?').get(req.sessionId, key);
  if (existing) {
    if (existing.request_hash !== requestHash) return res.status(409).json({ error: 'idempotency_key_reused' });
    try { return res.json(JSON.parse(existing.response_json)); } catch { return res.status(500).json({ error: 'idempotency_corrupt' }); }
  }
  const result = core.castVotes({ sessionId: req.sessionId, ip: ip(req), ua: req.headers['user-agent'], leaderSlug: slug, count });
  if (result.error) {
  ...
  db.prepare('INSERT INTO vote_idempotency (session_id,idempotency_key,request_hash,response_json) VALUES (?,?,?,?)').run(req.sessionId, key, requestHash, JSON.stringify(result));
  db.prepare("DELETE FROM vote_idempotency WHERE created_at < datetime('now','-2 days')").run();
  res.json(result);
});

router.post('/share', rateLimit({ windowMs: 60_000, max: 10, name: 'share-endpoint' }), (req, res) => {
...
router.post('/referral', rateLimit({ windowMs: 60 * 60_000, max: 20, name: 'referral-endpoint' }), (req, res) => {
  const { shareId, slug } = req.body || {};
  if (!shareId) return res.json({ ok: false });
  const chk = fraud.checkReferral({ shareId: String(shareId).slice(0, 24), visitorIp: ip(req), ownerSession: req.sessionId });
  ...
  db.prepare('INSERT INTO referrals (share_id,visitor_session,visitor_ip_hash,leader_id) VALUES (?,?,?,?)').run(String(shareId).slice(0, 24), req.sessionId, chk.ipHash, leader ? leader.id : null);
  db.prepare('UPDATE shares SET clicks=clicks+1 WHERE id=?').run(String(shareId).slice(0, 24));
  res.json({ ok: true });
});
...
router.post('/purchase/intent', rateLimit({ windowMs: 10 * 60_000, max: 10, name: 'purchase-intent' }), (req, res) => {
  ...
  if (kind === 'ad' && !db.prepare('SELECT 1 FROM advertising_slots WHERE id=? AND active=1').get(reference)) return res.status(404).json({ error: 'slot_not_found' });
  if (kind === 'anthem' && !db.prepare('SELECT 1 FROM countries WHERE code=?').get(String(reference || '').toUpperCase())) return res.status(404).json({ error: 'country_not_found' });
  if (kind === 'votes' && !VOTE_PACKS[reference]) return res.status(400).json({ error: 'pack_not_found' });
  const amountUsd = kind === 'votes' ? VOTE_PACKS[reference].usd : kind === 'ad' ? 5 : 5;
  const ccy = currency.guessCurrency(req.headers['accept-language']);
  const intent = payments.createIntent({ sessionId: req.sessionId, kind, reference, advertiser: cleanText(advertiser, 60), amountUsd });
  const termsMap = {
    ad: { item: `Advertising slot: ${reference}`, duration: 'Slot ownership follows the published slot terms.', receives: 'Sponsored placement on Global Leaders Live.' },
    anthem: { item: `National anthem sponsorship: ${reference}`, duration: 'Ownership lasts until another buyer takes over the same slot.', receives: 'Sponsored-by credit on the country and anthem pages.' },
    votes: { item: `Vote pack: ${VOTE_PACKS[reference] ? VOTE_PACKS[reference].votes : ''} votes`, duration: 'Votes are credited to your session instantly and never expire.', receives: `${VOTE_PACKS[reference] ? VOTE_PACKS[reference].votes : ''} extra votes.` }
  };
  res.json({ ...intent, amountUsd, priceDisplay: currency.display(amountUsd, ccy), terms: { ...termsMap[kind], price: `$${amountUsd.toFixed(2)} USD`, refunds: 'Refunds follow the applicable Payment Terms.' }, demoMode: core.getSetting('demo_mode') === '1' });
});
...
router.post('/purchase/confirm', rateLimit({ windowMs: 10 * 60_000, max: 15, name: 'purchase-confirm' }), (req, res) => {
  ...
  if (p.kind === 'ad') {
    const slotId = p.reference; const d = details || {}; const advertiser = cleanText(d.advertiser || 'Anonymous sponsor', 60) || 'Anonymous sponsor'; const xh = cleanX(d.x_handle);
    let imagePath = null;
    ...
    const ad = db.prepare(`INSERT INTO advertisements (slot_id,advertiser,image,text,cta,url,x_handle,starts_at,status) VALUES (?,?,?,?,?,?,?,datetime('now'),'active')`).run(slotId, advertiser, imagePath, cleanText(d.text, 120) || '', cleanText(d.cta, 30) || 'Learn more', sanitizeUrl(d.url), xh);
    db.prepare('INSERT INTO ad_purchases (slot_id,ad_id,payment_id,advertiser,amount_usd) VALUES (?,?,?,?,5.0)').run(slotId, ad.lastInsertRowid, p.id, advertiser);
    core.pushActivity('ad', `📢 ${advertiser}${xh ? ' (@' + xh + ')' : ''} took over the ${slotId.replace('-',' ')} ad space`, null, null); sse.broadcast('ad_purchased', { slotId, advertiser });
    return res.json({ ok: true, kind: 'ad', slotId, advertiser, shareText: `🚀 ${advertiser} now owns a live advertising position on Global Leaders Live.` });
  }
  if (p.kind === 'anthem') {
    const cc = String(p.reference).toUpperCase(); const sponsor = cleanText((details || {}).sponsor, 60) || 'Anonymous'; const xh = cleanX((details || {}).x_handle);
    const prev = db.prepare('SELECT sponsor FROM anthem_slots WHERE country_code=?').get(cc);
    db.prepare(`INSERT INTO anthem_slots (country_code,sponsor,sponsor_session,price_usd,purchased_at,sponsor_x) VALUES (?,?,?,5.0,datetime('now'),?) ON CONFLICT(country_code) DO UPDATE SET sponsor=excluded.sponsor,sponsor_session=excluded.sponsor_session,purchased_at=excluded.purchased_at,sponsor_x=excluded.sponsor_x`).run(cc, sponsor, req.sessionId, xh);
    ...
    core.pushActivity('anthem', `${core.FLAG(cc)} ${sponsor}${xh ? ' (@' + xh + ')' : ''} took over ${cname}'s national anthem`, cc, null); sse.broadcast('anthem_purchased', { country: cc, sponsor, sponsor_x: xh });
    return res.json({ ok: true, kind: 'anthem', country: cc, sponsor, shareText: `${core.FLAG(cc)} I just took over ${cname}'s national anthem slot on Global Leaders Live!` });
  }
  if (p.kind === 'votes') {
    const pack = VOTE_PACKS[p.reference]; if (!pack) return res.status(400).json({ error: 'pack_not_found' });
    const vs = core.getOrCreateVoteSession(req.sessionId, ip(req), req.headers['user-agent']);
...
router.post('/webhooks/:provider', express.raw({ type: '*/*' }), (req, res) => {
  const evt = payments.webhook(req.params.provider, req.body, req.headers); res.json({ received: true, handled: !!evt });
});
router.get('/ads', (req, res) => {
  const slots = db.prepare('SELECT id,label,price_usd FROM advertising_slots WHERE active=1').all();
  const ads = db.prepare(`SELECT slot_id,advertiser,image,text,cta,url,x_handle,created_at FROM advertisements WHERE status='active' AND (ends_at IS NULL OR ends_at > datetime('now')) AND (starts_at IS NULL OR starts_at <= datetime('now'))`).all();
